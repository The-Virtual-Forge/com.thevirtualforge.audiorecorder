...
#define TAG "com.thevirtualforge.audiorecorder"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> AudioRecorder_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("AudioRecorder.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::AudioRecorderBindings::lookupGeneratedInit(
...
static void AudioRecorder_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(AudioRecorder_getBinding)->GetFunction());
}

static void AudioRecorder_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::AudioRecorderBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry AudioRecorderBinding = {
	"com.thevirtualforge.audiorecorder",
	AudioRecorder_init,
	AudioRecorder_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_thevirtualforge_audiorecorder_AudioRecorderBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.thevirtualforge.audiorecorder", &AudioRecorderBinding);
	titanium::KrollBindings::addExternalLookup(&(::AudioRecorderBindings::lookupGeneratedInit));
}
